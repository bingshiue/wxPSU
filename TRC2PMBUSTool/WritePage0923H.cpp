...
#define SELF_CMD  0x0923/**< Self Command */

#define DEFAULT_HIGH_BYTE_VALUE    445/**< Default high byte Value */
#define DEFAULT_LOW_BYTE_VALUE    0x00/**< Default low  byte Value */

#define WRITE_PAGES_0923H_DEFAULT_FORMAT_HEX  FALSE//TRUE//FALSE

#define MAX_INPUT_VALUE   1000/**< Max Input Value */
#define MIN_INPUT_VALUE    100/**< Min Input Value */

WritePage0923H::WritePage0923H(wxWindow* parent, wxString& label, bool* monitor_running, std::vector<PMBUSSendCOMMAND_t> *sendCMDVector, IOACCESS* ioaccess, unsigned int* currentIO) : BaseWritePage(parent, label) {
	...
	wxString descriptionStr = wxString::Format(L"輸入範圍 : %d(0x%04X) ~ %d(0x%04X)",
...
#if WRITE_PAGES_0923H_DEFAULT_FORMAT_HEX == TRUE
	// Set Default Value of Radio Buttons
	this->m_cookRadioButton->SetValue(false);
	this->m_rawRadioButton->SetValue(true);

	wxString hexString_high_byte = wxString::Format("%02lx", (long)PMBUSHelper::GetAppSettings()->m_pbf00300gOption.m_0923WritePageInput);//DEFAULT_HIGH_BYTE_VALUE);
	...
	wxString default_high_byte = wxString::Format("%d", PMBUSHelper::GetAppSettings()->m_pbf00300gOption.m_0923WritePageInput);//DEFAULT_HIGH_BYTE_VALUE);
...
WritePage0923H::~WritePage0923H() {

}

void WritePage0923H::changeLayOutByDataFormat(unsigned int dataFormat, PMBUSCOMMAND_t *pmbuscmd) {
...
void WritePage0923H::OnRadioButtonCook(wxCommandEvent& event) {
...
void WritePage0923H::OnRadioButtonRaw(wxCommandEvent& event) {
	PSU_DEBUG_PRINT(MSG_DEBUG, "");
...
void WritePage0923H::OnTargetValue(wxCommandEvent& event) {
	PSU_DEBUG_PRINT(MSG_DEBUG, "");
...
#define CMD_0923H_BYTES_TO_READ  6/**< Bytes To Read */
void WritePage0923H::OnButtonWrite(wxCommandEvent& event) {
	PSU_DEBUG_PRINT(MSG_DEBUG, "");
	...
	if (highByteValue > PMBUSHelper::GetAppSettings()->m_pbf00300gOption.m_0923WritePageMax || highByteValue < PMBUSHelper::GetAppSettings()->m_pbf00300gOption.m_0923WritePageMin) {

		wxString msg = wxString::Format("Warning ! \n\n Input Value Must be in Range of \n %d ~ %d", PMBUSHelper::GetAppSettings()->m_pbf00300gOption.m_0923WritePageMin, PMBUSHelper::GetAppSettings()->m_pbf00300gOption.m_0923WritePageMax);

		wxMessageBox(msg,
			wxT("Warning ! Input Value Out of Range !"),
			wxOK | wxICON_WARNING);

		highByteValue = PMBUSHelper::GetAppSettings()->m_pbf00300gOption.m_0923WritePageDefaultInput;//DEFAULT_HIGH_BYTE_VALUE;
	...
	PMBUSHelper::GetAppSettings()->m_pbf00300gOption.m_0923WritePageInput = highByteValue;
	...
	PMBUSSendCOMMAND_t CMD0923H;

	CMD0923H.m_sendCommand = SELF_CMD;
	CMD0923H.m_sendDataLength = (*this->m_currentIO == IOACCESS_SERIALPORT || *this->m_currentIO == IOACCESS_TOTALPHASE) ? sendDataLength : 64;
	CMD0923H.m_bytesToRead = PMBUSHelper::GetBytesToReadOfWriteCMD(*this->m_currentIO, CMD_0923H_BYTES_TO_READ);
	for (unsigned idx = 0; idx < sizeof(SendBuffer) / sizeof(SendBuffer[0]); idx++) {
		CMD0923H.m_sendData[idx] = SendBuffer[idx];
	}

	if (*this->m_monitor_running == true) {
		if (this->m_sendCMDVector->size() == 0) {
			this->m_sendCMDVector->push_back(CMD0923H);
		...
		new(TP_SendWriteCMDTask) SendWriteCMDTask(m_ioaccess, m_currentIO, CMD0923H, true);
	}

}

wxBEGIN_EVENT_TABLE(WritePage0923H, wxPanel)
EVT_RADIOBUTTON(CID_RADIO_BOX_COOK, WritePage0923H::OnRadioButtonCook)
EVT_RADIOBUTTON(CID_RADIO_BOX_RAW, WritePage0923H::OnRadioButtonRaw)
EVT_TEXT(CID_TEXTCTRL_TARGET_VALUE, WritePage0923H::OnTargetValue)
EVT_BUTTON(CID_BUTTON_WRITE, WritePage0923H::OnButtonWrite)
wxEND_EVENT_TABLE()
