...
WritePageC0H::WritePageC0H(wxWindow* parent, wxString& label, bool* monitor_running, std::vector<PMBUSSendCOMMAND_t> *sendCMDVector, IOACCESS* ioaccess, unsigned int* currentIO) : BaseWritePage(parent, label){
...
WritePageC0H::~WritePageC0H(){

}

void WritePageC0H::changeLayOutByDataFormat(unsigned int dataFormat, PMBUSCOMMAND_t *pmbuscmd){
...
void WritePageC0H::OnRadioButtonCook(wxCommandEvent& event){
...
void WritePageC0H::OnRadioButtonRaw(wxCommandEvent& event){
	PSU_DEBUG_PRINT(MSG_DEBUG, "");
...
#define CMD_C0H_BYTES_TO_READ  6/**< Bytes To Read */
void WritePageC0H::OnButtonWrite(wxCommandEvent& event){
	PSU_DEBUG_PRINT(MSG_DEBUG, "");
	...
	case cmd_data_format_DirectData_Format:
		PMBUSHelper::ProductDirectData(mfrMaxTempArray, (double)mfrMaxTemp,
			&(PMBUSHelper::getPMBUSCMDData()[PMBUSHelper::getIndexOfCMD(PMBUSCMD_C0H_MFR_MAX_TEMP_1)])
			);
	...
	PMBUSSendCOMMAND_t CMDC0H;

	CMDC0H.m_sendDataLength = (*this->m_currentIO == IOACCESS_SERIALPORT || *this->m_currentIO == IOACCESS_TOTALPHASE) ? sendDataLength : 64;// sizeof(SendBuffer) / sizeof(SendBuffer[0]);
	CMDC0H.m_bytesToRead = PMBUSHelper::GetBytesToReadOfWriteCMD(*this->m_currentIO, CMD_C0H_BYTES_TO_READ);//(*this->m_currentIO == IOACCESS_SERIALPORT) ? CMD_C0H_BYTES_TO_READ : CMD_C0H_BYTES_TO_READ + 1;
	for (unsigned idx = 0; idx < sizeof(SendBuffer) / sizeof(SendBuffer[0]); idx++){
		CMDC0H.m_sendData[idx] = SendBuffer[idx];
	}
	...
	if (*this->m_monitor_running == true){
		...
		new(TP_SendWriteCMDTask) SendWriteCMDTask(m_ioaccess, m_currentIO, CMDC0H);
	}

}

wxBEGIN_EVENT_TABLE(WritePageC0H, wxPanel)
EVT_RADIOBUTTON(CID_RADIO_BOX_COOK, WritePageC0H::OnRadioButtonCook)
EVT_RADIOBUTTON(CID_RADIO_BOX_RAW, WritePageC0H::OnRadioButtonRaw)
EVT_BUTTON(CID_BUTTON_WRITE, WritePageC0H::OnButtonWrite)
wxEND_EVENT_TABLE()
