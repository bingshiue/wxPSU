...
#define SELF_CMD  0x0909/**< Self Command */

#define DEFAULT_HIGH_BYTE_VALUE   0x00/**< Default high byte Value */
#define DEFAULT_LOW_BYTE_VALUE    0x00/**< Default low  byte Value */

#define WRITE_PAGES_0909H_DEFAULT_FORMAT_HEX  FALSE//TRUE//FALSE

WritePage0909H::WritePage0909H(wxWindow* parent, wxString& label, bool* monitor_running, std::vector<PMBUSSendCOMMAND_t> *sendCMDVector, IOACCESS* ioaccess, unsigned int* currentIO) : BaseWritePage(parent, label){

	//
	this->m_cookRadioButton->Enable(false);
	this->m_rawRadioButton->Enable(false);
	this->m_writeButton->SetLabel(wxT("Reset"));
...
#if WRITE_PAGES_0909H_DEFAULT_FORMAT_HEX == TRUE
...
WritePage0909H::~WritePage0909H(){

}

void WritePage0909H::changeLayOutByDataFormat(unsigned int dataFormat, PMBUSCOMMAND_t *pmbuscmd){
...
#if 0
void WritePage0909H::OnRadioButtonCook(wxCommandEvent& event){
...
void WritePage0909H::OnRadioButtonRaw(wxCommandEvent& event){
	PSU_DEBUG_PRINT(MSG_DEBUG, "");
...
#define CMD_0909H_BYTES_TO_READ  6/**< Bytes To Read */
void WritePage0909H::OnButtonWrite(wxCommandEvent& event){
	...
	for (unsigned idx = 0; idx < sizeof(SendBuffer) / sizeof(SendBuffer[0]); idx++){
		CMD0909H.m_sendData[idx] = SendBuffer[idx];
	}

	if (*this->m_monitor_running == true){
		if (this->m_sendCMDVector->size() == 0){
			this->m_sendCMDVector->push_back(CMD0909H);
...
wxBEGIN_EVENT_TABLE(WritePage0909H, wxPanel)
//EVT_RADIOBUTTON(CID_RADIO_BOX_COOK, WritePage0909H::OnRadioButtonCook)
//EVT_RADIOBUTTON(CID_RADIO_BOX_RAW, WritePage0909H::OnRadioButtonRaw)
EVT_BUTTON(CID_BUTTON_WRITE, WritePage0909H::OnButtonWrite)
wxEND_EVENT_TABLE()
