...
void WritePage02H::OnButtonWrite(wxCommandEvent& event){
	...
	if (this->m_rawRadioButton->GetValue() == true){
		...
		PSU_DEBUG_PRINT(MSG_DEBUG, "Select Raw, Value = %d", cmdOnOffConfigValue);
	}
	else if (this->m_cookRadioButton->GetValue() == true){
		cmdOnOffConfigValue = wxAtoi(this->m_inputValue->GetValue());
		PSU_DEBUG_PRINT(MSG_DEBUG, "Select Cook, Value = %d", cmdOnOffConfigValue);
	}
	...
	CMD02H.m_sendDataLength = (*this->m_currentIO == IOACCESS_SERIALPORT || *this->m_currentIO == IOACCESS_TOTALPHASE) ? sendDataLength : 64;//sizeof(SendBuffer) / sizeof(SendBuffer[0]);
	CMD02H.m_bytesToRead = PMBUSHelper::GetBytesToReadOfWriteCMD(*this->m_currentIO, CMD_02H_BYTES_TO_READ);//(*this->m_currentIO == IOACCESS_SERIALPORT) ? CMD_02H_BYTES_TO_READ : CMD_02H_BYTES_TO_READ+1;
