...
void WritePage51H::changeLayOutByDataFormat(unsigned int dataFormat, PMBUSCOMMAND_t *pmbuscmd){
...
void WritePage51H::OnButtonWrite(wxCommandEvent& event){
	...
	case cmd_data_format_DirectData_Format:
		PMBUSHelper::ProductDirectData(otWarnLimitValueArray, (double)otWarnLimitValue,
			&(PMBUSHelper::getPMBUSCMDData()[PMBUSHelper::getIndexOfCMD(PMBUSCMD_51H_OT_WARN_LIMIT)])
			);
	...
	CMD51H.m_sendDataLength = (*this->m_currentIO == IOACCESS_SERIALPORT || *this->m_currentIO == IOACCESS_TOTALPHASE) ? sendDataLength : 64;// sizeof(SendBuffer) / sizeof(SendBuffer[0]);
	CMD51H.m_bytesToRead = PMBUSHelper::GetBytesToReadOfWriteCMD(*this->m_currentIO, CMD_51H_BYTES_TO_READ);//(*this->m_currentIO == IOACCESS_SERIALPORT) ? CMD_51H_BYTES_TO_READ : CMD_51H_BYTES_TO_READ+1;
