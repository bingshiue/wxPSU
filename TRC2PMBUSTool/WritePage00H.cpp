...
WritePage00H::WritePage00H(wxWindow* parent, wxString& label, bool* monitor_running, std::vector<PMBUSSendCOMMAND_t> *sendCMDVector, IOACCESS* ioaccess, unsigned int* currentIO) : BaseWritePage(parent, label){
	...
	wxString hexString = wxString::Format("%02x", DEFULT_PAGE);
...
void WritePage00H::OnButtonWrite(wxCommandEvent& event){
	...
	unsigned char changePageSendBuffer[64];
	unsigned int sendDataLength = PMBUSHelper::ProductWriteCMDBuffer(
		m_currentIO, 
		changePageSendBuffer, 
		sizeof(changePageSendBuffer),
		0x00, // CMD
		&cmdPageValue,
		sizeof(cmdPageValue)
		);
	...
	CMD00H.m_bytesToRead = PMBUSHelper::GetBytesToReadOfWriteCMD(*this->m_currentIO, CMD_00H_BYTES_TO_READ);//(*this->m_currentIO == IOACCESS_SERIALPORT) ? CMD_00H_BYTES_TO_READ : CMD_00H_BYTES_TO_READ+1;
