...
WritePage9AH::WritePage9AH(wxWindow* parent, wxString& label, bool* monitor_running, std::vector<PMBUSSendCOMMAND_t> *sendCMDVector, IOACCESS* ioaccess, unsigned int* currentIO) : BaseWritePage(parent, label){
	...
	m_inputValue = new wxTextCtrl(this, wxID_ANY, wxEmptyString, wxDefaultPosition, wxSize(100, -1));
	...
	this->m_inputValue->SetValue(PMBUSHelper::GetAppSettings()->m_mfr_model);
...
void WritePage9AH::OnButtonWrite(wxCommandEvent& event){
	...
	PMBUSHelper::GetAppSettings()->m_mfr_model = input_mfr_model;
...
void WritePage9AH::OnButtonLoadDefault(wxCommandEvent& event){
	PSU_DEBUG_PRINT(MSG_DEBUG, "");

	this->m_inputValue->SetValue(PMBUSHelper::getDefaultMFR_MODEL());
}
...
EVT_BUTTON(CID_BUTTON_LOADDEFAULT, WritePage9AH::OnButtonLoadDefault)
wxEND_EVENT_TABLE()
