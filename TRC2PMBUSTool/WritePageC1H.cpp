...
WritePageC1H::WritePageC1H(wxWindow* parent, wxString& label, bool* monitor_running, std::vector<PMBUSSendCOMMAND_t> *sendCMDVector, IOACCESS* ioaccess, unsigned int* currentIO) : BaseWritePage(parent, label){
...
WritePageC1H::~WritePageC1H(){

}

void WritePageC1H::changeLayOutByDataFormat(unsigned int dataFormat, PMBUSCOMMAND_t *pmbuscmd){
...
void WritePageC1H::OnButtonWrite(wxCommandEvent& event){
	...
	case cmd_data_format_DirectData_Format:
		PMBUSHelper::ProductDirectData(mfrMaxTempArray, (double)mfrMaxTemp,
			&(PMBUSHelper::getPMBUSCMDData()[PMBUSHelper::getIndexOfCMD(PMBUSCMD_C1H_MFR_MAX_TEMP_2)])
			);
	...
	PMBUSSendCOMMAND_t CMDC1H;

	CMDC1H.m_sendDataLength = (*this->m_currentIO == IOACCESS_SERIALPORT || *this->m_currentIO == IOACCESS_TOTALPHASE) ? sendDataLength : 64;// sizeof(SendBuffer) / sizeof(SendBuffer[0]);
	CMDC1H.m_bytesToRead = PMBUSHelper::GetBytesToReadOfWriteCMD(*this->m_currentIO, CMD_C1H_BYTES_TO_READ);//(*this->m_currentIO == IOACCESS_SERIALPORT) ? CMD_C1H_BYTES_TO_READ : CMD_C1H_BYTES_TO_READ + 1;
	for (unsigned idx = 0; idx < sizeof(SendBuffer) / sizeof(SendBuffer[0]); idx++){
		CMDC1H.m_sendData[idx] = SendBuffer[idx];
	}

	if (*this->m_monitor_running == true){
		if (this->m_sendCMDVector->size() == 0){
			this->m_sendCMDVector->push_back(CMDC1H);
		...
		new(TP_SendWriteCMDTask) SendWriteCMDTask(m_ioaccess, m_currentIO, CMDC1H);
	}

}

wxBEGIN_EVENT_TABLE(WritePageC1H, wxPanel)
EVT_RADIOBUTTON(CID_RADIO_BOX_COOK, WritePageC1H::OnRadioButtonCook)
EVT_RADIOBUTTON(CID_RADIO_BOX_RAW, WritePageC1H::OnRadioButtonRaw)
EVT_BUTTON(CID_BUTTON_WRITE, WritePageC1H::OnButtonWrite)
wxEND_EVENT_TABLE()
