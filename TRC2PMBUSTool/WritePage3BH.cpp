...
WritePage3BH::WritePage3BH(wxWindow* parent, wxString& label, bool* monitor_running, std::vector<PMBUSSendCOMMAND_t> *sendCMDVector, IOACCESS* ioaccess, unsigned int* currentIO) : BaseWritePage(parent, label){
	...
	wxString hexString = wxString::Format("%02lx", DEFAULT_DIAG_VALUE);
...
void WritePage3BH::OnButtonWrite(wxCommandEvent& event){
	...
	if (this->m_rawRadioButton->GetValue() == true){
		...
		PSU_DEBUG_PRINT(MSG_DEBUG, "Select Raw, Value = %d", DiagValue);
	}
	else if (this->m_cookRadioButton->GetValue() == true){
		DiagValue = wxAtoi(this->m_inputValue->GetValue());
		PSU_DEBUG_PRINT(MSG_DEBUG, "Select Cook, Value = %d", DiagValue);
	}
	...
	CMD3BH.m_sendDataLength = (*this->m_currentIO == IOACCESS_SERIALPORT || *this->m_currentIO == IOACCESS_TOTALPHASE) ? sendDataLength : 64;//sizeof(SendBuffer) / sizeof(SendBuffer[0]);
	CMD3BH.m_bytesToRead = PMBUSHelper::GetBytesToReadOfWriteCMD(*this->m_currentIO, CMD_3BH_BYTES_TO_READ);//(*this->m_currentIO == IOACCESS_SERIALPORT) ? CMD_3BH_BYTES_TO_READ : CMD_3BH_BYTES_TO_READ + 1;
