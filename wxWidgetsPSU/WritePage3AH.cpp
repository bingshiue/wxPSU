/**
 * @file WritePage3AH.cpp
 */

#include "PMBUSCMDWritePages.h"

#define DEFAULT_VALUE 0xD4

WritePage3AH::WritePage3AH(wxWindow* parent, wxString& label, bool* monitor_running, std::vector<PMBUSSendCOMMAND_t> *sendCMDVector, IOACCESS* ioaccess, unsigned int* currentIO) : BaseWritePage(parent, label){
...
void WritePage3AH::OnButtonWrite(wxCommandEvent& event){
	...
	unsigned char SendBuffer[8] = {
		0x41, 0x54, 0xB6, 0x3A, fan12ConfigValue, 0x00, 0x0D, 0x0A
	};
	...
	if (*this->m_monitor_running == true){
		if (this->m_sendCMDVector->size() == 0){
			this->m_sendCMDVector->push_back(CMD3AH);
		...
		new(TP_SendWriteCMDTask)SendWriteCMDTask(m_ioaccess, m_currentIO, CMD3AH);
	}

}

wxBEGIN_EVENT_TABLE(WritePage3AH, wxPanel)
EVT_RADIOBUTTON(CID_RADIO_BOX_COOK, WritePage3AH::OnRadioButtonCook)
EVT_RADIOBUTTON(CID_RADIO_BOX_RAW, WritePage3AH::OnRadioButtonRaw)
EVT_BUTTON(CID_BUTTON_WRITE, WritePage3AH::OnButtonWrite)
wxEND_EVENT_TABLE()