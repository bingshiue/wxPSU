/**
 * @file ReadPage1BH.cpp
 */
#include "PMBUSCMDReadPages.h"

#define DEFAULT_CMD   0x78/**< Default CMD */

ReadPage1BH::ReadPage1BH(wxWindow* parent, wxString& label, bool* monitor_running, std::vector<PMBUSSendCOMMAND_t> *sendCMDVector, IOACCESS* ioaccess, unsigned int* currentIO) : BaseReadPage(parent, label){
	// Initial Input Fields
	m_cmd = new wxStaticText(this, wxID_ANY, wxString(L"STATUS_x CMD"), wxDefaultPosition, wxSize(-1, -1));
...
ReadPage1BH::~ReadPage1BH(){

}


void ReadPage1BH::OnRadioButtonCook(wxCommandEvent& event){
...
void ReadPage1BH::OnRadioButtonRaw(wxCommandEvent& event){
	PSU_DEBUG_PRINT(MSG_DEBUG, "");
...
#define SELF_CMD  0x1b
void ReadPage1BH::OnButtonSet(wxCommandEvent& event){
	...
	PSU_DEBUG_PRINT(MSG_ALERT, "Set SMBALERT_MASK CMD to %02xH", cmd);

}

wxBEGIN_EVENT_TABLE(ReadPage1BH, wxPanel)
EVT_RADIOBUTTON(CID_RADIO_BOX_COOK, ReadPage1BH::OnRadioButtonCook)
EVT_RADIOBUTTON(CID_RADIO_BOX_RAW, ReadPage1BH::OnRadioButtonRaw)
EVT_BUTTON(CID_BUTTON_SET, ReadPage1BH::OnButtonSet)
wxEND_EVENT_TABLE()