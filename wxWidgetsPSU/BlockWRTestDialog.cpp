...
void BlockWRTestDialog::OnBtnSTART(wxCommandEvent& event){
	...
	if (TaskEx::GetCount(task_ID_PMBUSBlockWRTestTask) == 0){
		...
		new(TP_PMBUSBlockWRTestTask)PMBUSBlockWRTestTask(this->m_ioaccess, this->m_currentIO, this->m_pmbusBlockWRCMDArray, this->m_setCountSelectCB->GetSelection(), &this->m_outputLog, intervalTime);
