...
#define DEFAULT_PAGE  0x01/**< Default PAGE */
...
WritePage05H::WritePage05H(wxWindow* parent, wxString& label, bool* monitor_running, std::vector<PMBUSSendCOMMAND_t> *sendCMDVector, IOACCESS* ioaccess, unsigned int* currentIO) : BaseWritePage(parent, label){
	...
	m_code = new wxStaticText(this, wxID_ANY, wxString(L"DATA 1"), wxDefaultPosition, wxSize(-1, -1));
	...
	m_mask = new wxStaticText(this, wxID_ANY, wxString(L"DATA 2"), wxDefaultPosition, wxSize(-1, -1));
...
void WritePage05H::OnButtonWrite(wxCommandEvent& event){
	...
	if (this->m_rawRadioButton->GetValue() == true){
		...
		PSU_DEBUG_PRINT(MSG_DEBUG, "Select Raw, cmd = %02x, data1 = %02x, data2 = %02x", cmd, code, mask);
	}
	// Cook
		...
		PSU_DEBUG_PRINT(MSG_DEBUG, "Select Raw, cmd = %d, data1 = %d, data2 = %d", cmd, code, mask);
	}

	unsigned char smbAlertValueArray[4];
	smbAlertValueArray[0] = page; // page
	smbAlertValueArray[1] = cmd;  // command code
	smbAlertValueArray[2] = code;
	smbAlertValueArray[3] = mask;
